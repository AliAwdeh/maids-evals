...
def review(request: Request, idx: int = 0):
    sid = get_or_create_session_id(request)
    sess = sessions.get(sid, {})
    results = sess.get("results") or []
    csv_cols = sess.get("csv_cols") or []
...
<body>
  <div class="nav">
    <a href="/export"><button type="button">Back to export</button></a>
    <span class="small">Row {idx + 1} of {total}</span>
    <a href="/review?idx={prev_idx}"><button type="button" {"disabled" if idx == 0 else ""}>Previous</button></a>
    <a href="/review?idx={next_idx}"><button type="button" {"disabled" if idx >= total - 1 else ""}>Next</button></a>
  </div>
  <div class="wrap">
    <div class="pane">
      <h3>Input columns</h3>
